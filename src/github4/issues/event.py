...
class IssueEvent(GitHubCore):
    ...
    :meth:`~github4.issues.issue.Issue.events` which calls
    ...
    .. attribute:: actor
        ...
        A :class:`~github4.users.ShortUser` representing the user who
    ...
    def _update_attributes(self, event):
        ...
        if self.review_requester:
            self.review_requester = users.ShortUser(self.review_requester, self)
        self.requested_reviewers = event.get("requested_reviewers")
        if self.requested_reviewers:
            self.requested_reviewers = [
                users.ShortUser(reviewer, self) for reviewer in self.requested_reviewers
            ]
...
class RepositoryIssueEvent(IssueEvent):
    """Representation of an issue event on the repository level.
        ...
        This object will be instantiated from calling
        :meth:`~github4.repos.repo.Repository.issue_events` or
        :meth:`~github4.repos.repo.ShortRepository.issue_events` which call
        https://developer.github.com/v3/issues/events/#list-events-for-a-repository
    github4
        See alsogithub4//developer.github.com/v3/issues/events

        This object has all of the attributes of
        :class:`~github4.issues.event.IssueEvent` and the following:

        .. attribute:: issue
    github4
            A :class:`~github4.issues.issue.ShortIssue` representing the issue

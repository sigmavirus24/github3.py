...
class _Team(models.GitHubCore):
    ...
    def add_or_update_membership(self, username, role="member"):
        ...
        if role not in self.member_roles:
            raise ValueError(
                "'role' must be one of {}".format(", ".join(sorted(self.member_roles)))
            )
        data = {"role": role}
    ...
    def repositories(self, number=-1, etag=None):
        ...
        :rtype:
            :class:`~github4.repos.ShortRepository`
        """
        headers = {"Accept": "application/vnd.github.ironman-preview+json"}
        url = self._build_url("repos", base_url=self._api)
        return self._iter(int(number), url, ShortRepository, etag=etag, headers=headers)
...
class Team(_Team):
    ...
    In addition to the attributes on a :class:`~github4.orgs.ShortTeam` a Team
    ...
    .. attribute:: organization
        ...
        A :class:`~github4.orgs.ShortOrganization` representing the
...
class _Organization(models.GitHubCore):
    ...
    PREVIEW_HEADERS = {"Accept": "application/vnd.github.hellcat-preview+json"}
    ...
    def create_hook(self, name, config, events=["push"], active=True):
        ...
        :returns:
            the created hook
        :rtype:
            :class:`~github4.orgs.OrganizationHook`
        """
        json = None
    ...
    def create_project(self, name, body=""):
        ...
        json = self._json(self._post(url, data, headers=Project.CUSTOM_HEADERS), 201)
    ...
    def create_repository(
        ...
        :rtype:
            :class:`~github4.repos.Repository`
        """
        url = self._build_url("repos", base_url=self._api)
        data = {
            "name": name,
    ...
    def create_team(
        ...
        headers = (
            self.PREVIEW_HEADERS if parent_team_id or privacy == "closed" else None
        )
        if parent_team_id:
    ...
    @requires_auth
    def invite(self, team_ids, invitee_id=None, email=None, role="direct_member"):
        ...
        :rtype:
            :class:`~github4.orgs.Invitation`
        """
        if (invitee_id is None and email is None) or (
            invitee_id is not None and email is not None
        ):
            raise ValueError("One of either 'invitee_id' or 'email' must be specified")
        if not team_ids:
            raise ValueError("'team_ids' must be a non-empty list of integers")
    ...
    def invitations(self, number=-1, etag=None):
        ...
        url = self._build_url("invitations", base_url=self._api)
        return self._iter(int(number), url, Invitation, etag=etag, headers=headers)
...
class Organization(_Organization):
    ...
    :meth:`~github4.github.GitHub.organization`. To provide a clear
    distinction between the types of orgs, github4.py uses different classes
    with different sets of attributes.

    .. versionchanged:: 1.0.0

    This object includes all attributes on
    :class:`~github4.orgs.ShortOrganization` as well as the following:
...
class Invitation(models.GitHubCore):
    ...
    .. attribute:: invitation_team_url
        ...
        The API URL to retrieve the :class:`~github4.orgs.ShortTeam` objects
        associated with this invitation.

    .. attribute:: inviter

        A :class:`~github4.users.ShortUser` representing the user who invited
...
class Membership(models.GitHubCore):
    ...
    .. attribute:: organization
        ...
        A :class:`~github4.orgs.ShortOrganization` instance representing the

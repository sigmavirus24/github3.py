...
class Project(models.GitHubCore):
    ...
    .. attribute:: creator
        ...
        A :class:`~github4.users.ShortUser` instance representing the user who
    ...
    def columns(self, number=-1, etag=None):
        ...
        :rtype:
            :class:`~github4.project.ProjectColumn`
        """
    ...
    def create_column(self, name):
        ...
        if name:
            json = self._json(
                self._post(url, data={"name": name}, headers=Project.CUSTOM_HEADERS),
                201,
            )
        return self._instance_or_null(ProjectColumn, json)
    ...
    def update(self, name=None, body=None):
        ...
        if data:
            json = self._json(
                self._patch(self._api, data=dumps(data), headers=self.CUSTOM_HEADERS),
                200,
            )
...
class ProjectColumn(models.GitHubCore):
    ...
    def cards(self, number=-1, etag=None):
        ...
        :returns:
            generator of cards
        :rtype:
            :class:`~github4.project.ProjectCard`
        """
    ...
    def create_card_with_note(self, note):
        ...
        if note:
            json = self._json(
                self._post(url, data={"note": note}, headers=Project.CUSTOM_HEADERS),
                201,
            )
        return self._instance_or_null(ProjectCard, json)
    ...
    def update(self, name=None):
        ...
        if data:
            url = self._build_url("projects", "columns", str(self.id))
            json = self._json(
                self._patch(url, data=dumps(data), headers=Project.CUSTOM_HEADERS),
                200,
            )
...
class ProjectCard(models.GitHubCore):
    ...
    def move(self, position, column_id):
        ...
        url = self._build_url("projects", "columns", "cards", str(self.id), "moves")
    ...
    def retrieve_issue_from_content(self):
        ...
        :raises:
            :class:`~github4.exceptions.CardHasNoContentUrl`
        """
        ...
        resp = self._get(self._build_url("repos", owner, repository, "issues", number))
    ...
    def retrieve_pull_request_from_content(self):
        ...
        parsed = self._uri_parse(self.content_url)
        _, owner, repository, _, number = parsed.path[1:].split("/", 5)
        resp = self._get(self._build_url("repos", owner, repository, "pulls", number))
        json = self._json(resp, 200)

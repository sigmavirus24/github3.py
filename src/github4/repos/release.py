...
class Release(models.GitHubCore):
    ...
    :class:`Repository <github4.repos.repo.Repository>`.
    ...
    .. attribute:: original_assets
        ...
        A list of :class:`~github4.repos.release.Asset` objects representing
    ...
    .. attribute:: author
        ...
        A :class:`~github4.users.ShortUser` representing the creator of this
        release.
    ...
    def asset(self, asset_id):
        ...
        if int(asset_id) > 0:
            i = self._api.rfind("/")
            url = self._build_url("assets", str(asset_id), base_url=self._api[:i])
...
class Asset(models.GitHubCore):
    ...
    .. seealso::
        ...
        `List Assets`_, :meth:`~github4.repos.release.Release.assets`
            Documentation around listing assets of a release
        `Upload Assets`_, :meth:`~github4.repos.release.Release.upload_asset`
            Documentation around uploading assets to a release
        `Get a Single Asset`_, :meth:`~github4.repos.release.Release.asset`
            Documentation around retrieving an asset
        `Edit an Asset`_, :meth:`~github4.repos.release.Asset.edit`
            Documentation around editing an asset
        `Delete an Asset`_, :meth:`~github4.repos.release.Asset.delete`
    ...
    def download(self, path=""):
        ...
        if resp.status_code == 302:
            ...
            with self.session.no_auth():
                resp = self._get(resp.headers["location"], stream=True, headers=headers)
